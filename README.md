...
## 한컴오피스가 필요한 이유와 순수 Python 변환

### 현재 변환기의 역할 분담

현재 파이프라인에서 한컴오피스와 Python은 다음과 같이 역할을 나눕니다.
...
HWP 바이너리 파일
  ...
  ├─ 한컴오피스 COM 자동화
  │    ├─ HML 추출
  │    └─ SVG 변환 실패 시 사용할 네이티브 HTML/GIF 생성
  │
  └─ Python 변환기
       ├─ HML 본문 분석
       ├─ 수식 → LaTeX/KaTeX
...
즉 현재 Python 코드가 직접 처리하는 핵심 구간은 **HML → HTML**입니다. 원본 `.hwp` 바이너리를 HML로 풀어내는 첫 단계와 네이티브 GIF 폴백 생성은 설치된 한컴오피스에 맡깁니다.

따라서 현재 상태에서 `.hwp`를 `hwp2html.py`에 바로 입력하려면 다음 조건이 필요합니다.

- Windows
- 한컴오피스 한글 설치
- 한컴오피스 COM 자동화를 실행할 수 있는 사용자 세션
- Python, `hwpapi`, `pywin32`

### 한컴오피스 없이 변환할 수 있는 경우

이미 HML 파일이 있다면 한컴오피스 없이 다음 명령으로 HTML을 만들 수 있습니다.

```powershell
python hml2html.py input.hml output.html
```

다만 이 경로에서는 한컴 네이티브 HTML에서 가져오는 GIF 폴백을 준비할 수 없습니다. GTree/SVG로 재구성할 수 없는 객체 그림은 누락되거나 자리 표시자로 남을 수 있습니다.

### 순수 Python만으로 HWP를 변환할 수 있는가?

기술적으로는 가능합니다. 그러나 현재 프로젝트에는 `.hwp` 바이너리 전체를 직접 읽는 파서가 포함되어 있지 않습니다. 한컴오피스를 제거하려면 `rhwp`, `pyhwp` 등의 구현을 참고하거나 별도의 HWP 바이너리 파서를 연결하여 다음 단계를 대체해야 합니다.

```text
현재 방식
HWP → 한컴오피스 COM → HML → 현재 Python 변환기 → HTML

순수 변환 방식
HWP → 독립 HWP 바이너리 파서 → 중간 문서 구조 → 현재 변환기 → HTML
```

본문 텍스트만 추출하는 것은 비교적 단순하지만, 현재 변환 품질을 유지하려면 다음 레코드와 속성을 모두 해석해야 합니다.

- 문단과 문자 모양
- 글꼴, 글자 크기, 위첨자와 아래첨자
- 표의 행·열, 셀 병합, 너비·높이, 여백, 테두리와 배경
- 한글 수식 스크립트와 수식 개체 연결
- 포함 이미지와 바이너리 데이터
- 객체 그림의 좌표계, 회전, 크기 조절, 이동 행렬
- 선, 곡선, 호, 화살표, 점선, 텍스트 상자 등의 도형 속성
- 각주와 기타 확장 컨트롤

특히 객체 그림은 한글 내부 좌표계와 변환 행렬을 정확히 복원해야 교차점과 점선 위치가 맞습니다. 또한 한컴오피스가 없으면 네이티브 GIF 폴백을 생성할 수 없으므로, 지원하지 않는 도형은 별도의 래스터라이저를 구현하지 않는 한 원본과 동일하게 표시하기 어렵습니다.

따라서 순수 Python 지원은 단순한 실행 옵션 변경이 아니라 **HWP 바이너리 파서와 렌더링 계층을 추가하는 별도 개발 단계**입니다.

### HWPX는 더 쉽게 처리할 수 있음

HWPX는 ZIP 컨테이너 안에 XML 문서와 이미지 리소스를 저장하는 형식이므로 `.hwp` 바이너리보다 순수 Python으로 처리하기 쉽습니다.

```text
HWPX → ZIP 해제 → XML·이미지 분석 → HTML
```

향후 한컴오피스가 없는 Linux 서버나 Docker 환경을 목표로 한다면 HWPX 직접 지원을 먼저 추가하는 것이 현실적인 경로입니다. 단, 기존 `.hwp`를 HWPX로 바꾸는 과정에는 여전히 한컴오피스 또는 독립 HWP 파서가 필요합니다.

### 웹앱 배포 시 선택지

#### 현재 품질을 우선하는 방식

Windows 서버 또는 Windows PC에 한컴오피스를 설치하고 현재 `hwp2html.py`를 사용합니다.

- 지금 구현된 표·수식·SVG 변환 품질 유지
- 변환 실패 그림의 GIF 폴백 사용 가능
- 여러 파일은 한컴 COM 충돌을 피하기 위해 기본적으로 순차 처리 권장

#### 한컴오피스 없는 서버를 우선하는 방식

독립 HWP/HWPX 파서를 추가하여 Linux 또는 Docker에서 실행합니다.

- 서버 배포와 자동 확장이 쉬움
- 한컴오피스 라이선스와 COM 사용자 세션에 의존하지 않음
- 복잡한 표·수식·객체 그림 지원을 다시 구현해야 함
- 네이티브 GIF 폴백을 대체할 렌더러 필요

요약하면, **현재 코드는 한컴오피스가 있으면 HWP부터 자동 변환하고, HML이 이미 있으면 Python만으로 HTML을 생성할 수 있습니다.** 한컴오피스 없이 원본 HWP부터 동일한 품질로 변환하려면 독립 HWP 바이너리 파서가 추가로 필요합니다.
